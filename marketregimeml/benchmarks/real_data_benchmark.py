...
class RealDataLoader:
    ...
    def load_crypto(
        self, symbol: str = "BTC/USD", limit: int = 500
        ...
        try:
            ...
            ohlcv = loader.fetch_ohlcv(
                symbol=symbol, timeframe="D", start=start_date, end=end_date  # Daily
            )
    ...
    def load_forex_data(
        self, symbol: str = "EUR_USD", timeframe: str = "D", limit: int = 500
    ) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            print(
                f"Loading {symbol} data from {start_date.date()} to {end_date.date()}"
            )
    ...
    def load_stock_data(
        self, symbol: str = "AAPL", timeframe: str = "daily", limit: int = 500
    ) -> Optional[pd.DataFrame]:
    ...
    def _prepare_features(self, ohlcv: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if os.getenv("MRML_USE_DENOISER", "0") == "1":
                from marketregimeml.preprocessing.denoiser_integration import (
                    denoise_series,
                )
                ...
                if y is not None and len(y) == len(ohlcv):
                    ...
                    core["returns_denoised"] = pd.Series(
                        y, index=ohlcv.index
                    ).pct_change()
        except Exception:
            pass
    ...
    def run(self, data: pd.DataFrame, dataset_name: str) -> BenchmarkResult:
        ...
        try:
            ...
            rqi = self.metrics.regime_quality_index(
                benchmark_data.values, predictions, proba
            )
...
class RealDataBenchmarkRunner:
    ...
    def run_all(self) -> List[BenchmarkResult]:
        ...
        for dataset_name, data in self.datasets.items():
            ...
            for model_name, model in self.models.items():
                ...
                if result.rqi:
                    print(
                        f"  ✓ Train: {result.train_time:.3f}s, "
    ...
    def generate_report(self, results: List[BenchmarkResult]) -> str:
        ...
        for dataset_name, dataset_results in datasets.items():
            ...
            dataset_results.sort(key=lambda x: x.rqi if x.rqi else 0, reverse=True)
            ...
            for result in dataset_results:
                ...
                if result == dataset_results[0] and result.rqi:
                    report.append(
                        f"| **{result.model_name}** | **{train}** | **{infer}** | **{rqi}** | **{stab}** |"
                    )
                else:
                    report.append(
                        f"| {result.model_name} | {train} | {infer} | {rqi} | {stab} |"
        ...
        for dataset_name, dataset_results in datasets.items():
            best = max(
                (r for r in dataset_results if r.rqi), key=lambda x: x.rqi, default=None
            )
            if best:
                report.append(
                    f"- **{dataset_name}**: {best.model_name} (RQI: {best.rqi:.1f})"
                )
        ...
        if fastest_train:
            report.append(
                f"- **Fastest Training**: {fastest_train.model_name} ({fastest_train.train_time:.3f}s)"
            )
        ...
        if fastest_infer:
            report.append(
                f"- **Fastest Inference**: {fastest_infer.model_name} ({fastest_infer.inference_time:.2f}ms)"
            )
...
def run_real_data_benchmark():
    ...
    models = {
        ...
        "MS-GARCH": MSGARCHRegimeDetector(
            n_regimes=2, random_state=42
        ),  # MS-GARCH works best with 2 regimes
