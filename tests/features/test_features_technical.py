...
class TestDataHelper:
    ...
    def create_ohlcv_data(n_periods=50, seed=42):
        ...
        volume = 1000000 * (1 + price_change * 10) * np.random.uniform(0.8, 1.2, n_periods)
    ...
    def create_trending_prices(n_periods=30, direction='up'):
        ...
        if direction == 'up':
            prices = 100 + np.arange(n_periods) * 0.5 + np.random.normal(0, 0.5, n_periods)
        elif direction == 'down':
            prices = 100 - np.arange(n_periods) * 0.5 + np.random.normal(0, 0.5, n_periods)
        else:  # sideways
...
class TestTechnicalIndicators:
    ...
    def test_bollinger_bands(self, indicators, price_data):
        ...
        upper, middle, lower = indicators.bollinger_bands(price_data, period=20, num_std=2)
...
class TestIndicatorRelationships:
    ...
    def all_indicators(self):
        ...
        return {
            ...
            'williams_r': indicators.williams_r(data['high'], data['low'], data['close']),
            'data': data
        }
