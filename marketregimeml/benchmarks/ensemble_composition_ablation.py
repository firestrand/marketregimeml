...
def test_ensemble_combinations(
    ...
    ensemble_candidates = (
        (["HMM", "GMM", "GARCH"] + ml_models) if include_unsupervised else ml_models
    )
...
def run_ensemble_ablation():
    ...
    for dataset_name, dataset_results in all_results.items():
        ...
        sorted_results = sorted(
            dataset_results.items(), key=lambda x: x[1]["rqi"], reverse=True
        )
        ...
        for ensemble_type in ["individual", "2-model", "3-model", "4-model"]:
            type_results = [
                (n, m) for n, m in sorted_results if m["type"] == ensemble_type
            ]
            if type_results:
                best = type_results[0]
