...
class TestRegimeReorderingUtils:
    ...
    def test_reorder_probabilities_wrong_dimensions(self):
        ...
        with pytest.raises(ValueError, match="Probability matrix has 3 columns, expected 2"):
    ...
    def test_validate_regime_labels_edge_cases(self):
        ...
        # Single regime
        assert RegimeReorderingUtils.validate_regime_labels(np.array([0, 0, 0]), n_regimes=1)
        
        # All same regime
        assert RegimeReorderingUtils.validate_regime_labels(np.array([1, 1, 1]), n_regimes=3)
